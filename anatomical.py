...
def anat_qc_workflow(dataset, settings, mod='T1w', name='anatMRIQC'):
    ...
    .. workflow::
        ...
        datadir = '/mnt/MD1200B/egarza/arun/Datasets/MONKEY_MRIQC/MONKEY_MRIQC/site-mcgill/'
        wf = anat_qc_workflow([op.join(datadir, 'site-mcgill_sub-032208_ses-001.nii.gz')],
                              settings={'bids_dir': datadir,
                                        'output_dir': op.abspath('/mnt/MD1200B/egarza/arun/Datasets/MONKEY_MRIQC/MONKEY_MRIQC/site-mcgill/outputMRIQC'),
                                        'ants_nthreads': 1,
                                        'no_sub': True})
    ...
    #experimentDir = 'locationOfMonkeyMri'
    #grabberT1Image = nipype.DataGrabber()
    #grabberT1Image-.inputs.base_directory = experimentDir + '/data'
    #grabberT1Image.inputs.subjectId = subjectName

    outputnode = pe.Node(niu.IdentityInterface(fields=['out_json']), name='outputnode')

    # 1. Reorient anatomical image {Done by Pamela}
    #to_ras = pe.Node(ConformImage(check_dtype=False), name='conform')
    to_ras = pe.Node(niu.IdentityInterface(fields=['out_file']), name='conform')
    locationOfConformedImage = datadir + 'reorient/sub-032208_ses-001_acq-mp2rage_run_1_T1w_REO.nii.gz' 
    to_ras.iterables = [('out_file'), locationOfConformedImage]

    # 2. Skull-stripping (afni) {Done by Pamela}
    #asw = skullstrip_wf(n4_nthreads=settings.get('ants_nthreads', 1), unifize=False)
    asw = pe.Node(niu.IdentityInterface(fields=['out_file','bias_corrected','out_mask','bias_image']), name='skullstrip')
    
    locationOfOutFile = datadir + 'N4_2_final_image/secondN4/out_file.nii.gz'
    locationOfBiasCorrected = datadir + 'N4_2_final_image/secondN4/bias_corrected.nii.gz'
    locationOfBiasImage = datadir + 'N4_2_final_image/secondN4/bias_image.nii.gz'
    locationOfOutMask = datadir + 'N4_2_final_image/secondN4/out_mask.nii.gz'

    asw.iterables = [('out_file', locationOfOutFile),('bias_corrected',locationOfBiasCorrected),('out_mask',locationOfOutMask),('bias_image',locationOfBiasImage)]
    ...
    # Connect all nodes
    workflow.connect([
        #(inputnode, to_ras, [('in_file', 'in_file')]),  Removed input to Ras file as Pamela will be doing this
...
def compute_iqms(settings, modality='T1w', name='ComputeIQMs'):
    ...
    #    label=['CSF', 'GM', 'WM'])]
    invt.inputs.input_image = ['/mnt/MD1200B/egarza/arun/Datasets/MONKEY_MRIQC/monkeyTemplates/NMT_segmentation_CSF.nii.gz',
        '/mnt/MD1200B/egarza/arun/Datasets/MONKEY_MRIQC/monkeyTemplates/NMT_segmentation_GM.nii.gz',
        '/mnt/MD1200B/egarza/arun/Datasets/MONKEY_MRIQC/monkeyTemplates/NMT_segmentation_WM.nii.gz']
...
def airmsk_wf(name='AirMaskWorkflow'):
    ...
    invt.inputs.input_image = '/mnt/MD1200B/egarza/arun/Datasets/MONKEY_MRIQC/MONKEY_MRIQC/head_mask_template.nii.gz' #CHANGES MADE HERE
